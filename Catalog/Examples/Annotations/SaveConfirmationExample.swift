...
private class SavePermissionViewController: PDFViewController {
    ...
    @objc func exit() {
        ...
        let exitAction = UIAlertAction(title: "Exit without save", style: .destructive) { _ in
            // Clear document cache when discarding unsaved changes.
            // This is required when not saving changes
            // to prevent stale cached renders from appearing when the document is reopened.
