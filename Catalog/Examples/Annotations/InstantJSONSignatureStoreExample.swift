...
/// https://pspdfkit.com/guides/ios/json/
@objc(PSCInstantJSONSignatureStore) private class InstantJSONSignatureStore: NSObject, SignatureStore {
    ...
    /// page with y increasing downwards. Therefore, PSPDFKit needs to know the page height in order to preserve the
    ...
    private func loadSignatures() -> [SignatureContainer] {
        ...
        let annotations: [Annotation] = annotationJSONDictionaries.compactMap {
            do {
                ...
                let annotation = try Annotation(fromInstantJSON: annotationJSONData, documentProvider: placeholderDocument.documentProviders[0])
                placeholderDocument.add(annotations: [annotation])

                if $0["type"] as? String == "pspdfkit/image" {
                    ...
                    do {
                        ...
                        try annotation.attachBinaryInstantJSONAttachment(fromDataProvider: attachmentDataProvider)
                    } catch {
                        print("InstantJSONSignatureStore: Couldn’t attach image data: \(error)")
                        return nil
                    }
                }
