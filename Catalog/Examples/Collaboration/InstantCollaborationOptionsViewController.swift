...
class InstantCollaborationOptionsViewController: UITableViewController {
    ...
    private lazy var sections: [Section] = {
        ...
        if configuration.allowJoiningExistingSessions {
            ...
            let joinSessionSection = Section(header: "Join an existing session", rows: joinSessionRows, footer: "Enter a document link's from PDF Viewer or PSPDFKit Catalog on another device, or from a web browser showing pspdfkit.com/instant/demo or web-examples.services.demo.pspdfkit.com.")
        ...
        self.dismiss(animated: true) {
            ...
            case .success(let barcode):
                if let url = URL(string: barcode) {
                    ...
                    self.showAlert(withTitle: "Couldn’t Join Group", message: "This is not a link. Please scan the QR code from one of the examples at https://web-examples.services.demo.pspdfkit.com/")
                }
