...
@MainActor public protocol WebExamplesAPIClientDelegate: AnyObject {
...
/// Interfaces with our Nutrient Web SDK examples server.
///
...
public class WebExamplesAPIClient: NSObject, URLSessionTaskDelegate {
    ...
    public func createNewSession(documentIdentifier: InstantExampleDocumentIdentifier, jwtParameters: [String: Any]? = nil, completion: @escaping (Result<InstantDocumentInfo, WebExamplesAPIClientError>) -> Void) {
        ...
        if let jwtParameters {
            let actualParams = ["jwtParameters": jwtParameters]
            do {
    ...
    public func urlSession(_ session: URLSession, task: URLSessionTask, didReceive challenge: URLAuthenticationChallenge, completionHandler: @escaping (URLSession.AuthChallengeDisposition, URLCredential?) -> Void) {
        // The URLSessions are created with main as the delegateQueue.
    ...
    @MainActor private func promptForHTTPBasicAuthenticationCredential(challenge: URLAuthenticationChallenge, completion: @escaping (URLCredential?) -> Void) {
        if let delegate {
            delegate.examplesAPIClient(self, didReceiveBasicAuthenticationChallenge: challenge, completion: completion)
        } else {
            completion(nil)
        }
    }
