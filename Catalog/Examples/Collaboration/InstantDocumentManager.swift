...
/// The primary reason behind introducing this is to have only one `InstantClient` instance per Nutrient Document Engine.
class InstantDocumentManager: NSObject, InstantClientDelegate {
    ...
    /// `InstantClient` instances used to connect to the various Nutrient Document Engine instances.
    /// Each Nutrient Document Engine should have only one `InstantClient` instance in use.
    ...
    ///
    /// - Parameters:
    ///   - documentInfo: Document info to use to download an editable document from a Nutrient Document Engine.
    ///   - observer: An object that should be notified of the completion/errors occurred while accessing the document
    ///   form the Nutrient Document Engine using `InstantClient`.
    ...
    func instantClient(_ instantClient: InstantClient, documentDescriptor: InstantDocumentDescriptor, didFailDownloadWithError error: Error) {
        if let trackedDocument = trackedDocument(for: instantClient, documentDescriptor: documentDescriptor),
           let observer = trackedDocument.observer {
            let documentInfo = InstantDocumentInfo(serverURL: trackedDocument.serverURL, url: trackedDocument.url, jwt: trackedDocument.JWT, documentId: trackedDocument.documentIdentifier, encodedDocumentId: trackedDocument.encodedDocumentId)
            // The document download from the Nutrient Document Engine can fail and in that case you should
    ...
    func instantClient(_ instantClient: InstantClient, documentDescriptor: InstantDocumentDescriptor, didFailSyncWithError error: Error) {
        if let trackedDocument = trackedDocument(for: instantClient, documentDescriptor: documentDescriptor),
           let observer = trackedDocument.observer {
            ...
            // For example, when `InstantDocumentManager.disableSyncing(for:) is called where any ongoing
            ...
            // The gateway used by our example server will close the long-polling requests used by Instant after a
            // minute, resulting in a 502 status code. Instant will automatically retry, so we don’t need to pass
            // this error on. The HTTP Status Code user info key is not supported public API. If you need this for
            // your own setup, let us know: https://www.nutrient.io/support/request
            if case InstantError.requestFailed = error, (error as NSError).userInfo["HTTP Status Code"] as? Int == 502 {
                return
            }
            let documentInfo = InstantDocumentInfo(serverURL: trackedDocument.serverURL, url: trackedDocument.url, jwt: trackedDocument.JWT, documentId: trackedDocument.documentIdentifier, encodedDocumentId: trackedDocument.encodedDocumentId)
            print("Instant Document Failed to Sync to the Nutrient Document Engine. Reason: \(error.localizedDescription)")
            observer.didFailSyncing(for: documentInfo, reason: "Please ensure that you are connected to the internet. Otherwise please restart the session with a fresh token (JWT).")
        }
    }
