...
open class CatalogViewController: BaseTableViewController {
    ...
    public init() {
        ...
        // Use a shorter title in the back button on versions before iOS 26. On iOS 26 and later, setting
        // this property overrides the default behavior of not showing text in the back button as long as
        // there’s enough space, but we want to match the default look by not including the text.
        if #unavailable(iOS 26) {
            navigationItem.backButtonTitle = "Catalog"
        }
    ...
    @objc func didTapDebugButtonItem(_ sender: UIBarButtonItem) {
        ...
        debugSheet.popoverPresentationController?.sourceItem = sender
